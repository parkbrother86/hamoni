# mtbench — 자체 서빙 실험 (핸드오프)

> **결론 먼저 (2026-08-14 실측 완료):**
> Tri-1.8B(Apache 2.0, EN↔KO↔JA↔ZH) + vLLM + 4080 Laptop 12GB 기준
> **fan-out 3(1언어 입력 → 3언어)에서 입력 채팅 ~100/s · p95 179ms**
> (45초 지속 106.8/s 검증). 단일 방향 기준으론 258 msg/s. 품질은 일반 대화체 합격 / **게임 슬랭은 전 모델
...
>
> 개발기 로컬에 이번 연구 **전체 종합 문서**(비용·프롬프트·아키텍처 포함)가
> `bench/RESEARCH.md` 에 있다 — 이 폴더 밖 맥락이 필요하면 그쪽을 본다.
> (`bench/` 는 `.gitignore` 정책상 레포에 없음)
...
### ★ fan-out 실측 — 1언어 입력 → 3언어 (실제 릴레이 형태)

`--fanout en,jp,cn`: 채팅 1건 = kr→en/jp/cn 동시 3요청, **3개가 전부 끝나야
그 채팅 완료**.

> **슬롯 = 동시 처리 슬롯(in-flight 제한)이지 유저 수도 배치 크기도 아니다.**
> "채팅은 무한히 밀려오는데 동시에 N건까지만 GPU 에 물린다"는 뜻.
> 정확한 정의와 오해 3종은 **[CAPACITY.md](CAPACITY.md) §1**.

| 슬롯 (동시 채팅) | API req/s | **입력 채팅/s** | p50 | p95 |
|---:|---:|---:|---:|---:|
| 1 | 29.1 | 9.7 | 94ms | 117ms |
| 4 | 105.9 | 35.3 | 103ms | 129ms |
| 8 | 174.6 | 58.2 | 118ms | 185ms |
| **16** | **307.2** | **102.4** | 138ms | **179ms** ← 최적 |
| 32 | 126.2 ↓ | 42.1 | 380ms | 1189ms ← 붕괴 |
| 64 | 80.5 | 26.9 | 1147ms | 3944ms |

**지속 검증** (슬롯 16, 45초): **106.8 채팅/s** 유지, 전반부 p95 172ms /
후반부 174ms, 에러 0 — 완전히 안정.

**★ 결론: 입력 채팅 ~100/s (슬롯 16 = API in-flight 48).**

주목할 점 세 개:
1. fan-out 3 의 API 처리량(307 req/s)이 단일 방향 측정(258 msg/s)보다 **높다**.
   3개 요청이 `Translate the following Korean text into ` 까지 프리픽스를
   공유해 vLLM prefix caching 이 먹고, 동시 도착이라 배칭에도 유리하기 때문.
   → **fan-out 은 손해가 아니라 오히려 배칭 친화적**이다.
2. 붕괴 지점이 슬롯 32(=API in-flight 96)로, 단일 방향 실험의 sweet spot
   (48~64)과 일치한다. **제한 대상은 채팅 수가 아니라 API in-flight**.
3. **슬롯을 늘리면 오히려 느려진다** (16→32 에서 처리량 반토막, p95 6배).
   덜 넣는 게 더 빠르다 — 이유는 [CAPACITY.md](CAPACITY.md) §2.
...
fan-out 3 실측 = **입력 채팅 ~100/s** (위 §fan-out 실측; 동시 채팅 16). 수신자가
몇 명이든 타겟 언어 수만큼만 번역하므로, 언어 3개면 350명 채널이든 3회다.
번역 캐시가 걸리면 GPU 요청은 여기서 더 줄어든다.
