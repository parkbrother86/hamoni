...
그 채팅 완료**. concurrency 단위가 '동시 채팅'이다.

| 동시 채팅 | API req/s | **입력 채팅/s** | p50 | p95 |
|---:|---:|---:|---:|---:|
...
**지속 검증** (동시 채팅 16, 45초): **106.8 채팅/s** 유지, 전반부 p95 172ms /
후반부 174ms, 에러 0 — 완전히 안정.

**★ 결론: 입력 채팅 ~100/s (동시 채팅 16 = API 동시 48).**
...
2. 붕괴 지점이 동시 채팅 32(=API 96 동시)로, 단일 방향 실험의 sweet spot
   (동시 48~64)과 일치한다. **제한해야 할 것은 채팅 수가 아니라 API in-flight**.
