...
# ⚠️ 참조값 출처 주의: 일반 문장은 DeepSeek V10 출력이지만, 슬랭은
# **사용자 확정 정답**이다.
#
# 경위 (2026-08-20 정정 → 2026-08-21 재정정): README 표에 DeepSeek 의 `ㄱㄱ`
# 출력이 "GG" 로 기록돼 있어 "DeepSeek 이 슬랭에서 틀린다" 고 적었으나,
# 현행 프로덕션 V10 으로 3회 재측정하니 전부 "Go go" — **맞힌다**. 기록값이
# 재현되지 않는다(구 V9 시절 값이거나 전사 오류). 그 서술은 근거가 없었다.
#
# 결론(참조를 ground truth 로 쓰지 말 것)은 유지되지만 근거 사례가 바뀐다 —
# DeepSeek 의 재현되는 슬랭 실패는 `ㅊㅊ`→"GG" 다(축하를 GG 로, 3/3 동일).
# 측정 하네스: bench/quality_slang.js (bench/ 는 로컬 전용, 레포에 없음)
QUALITY_CASES = [
